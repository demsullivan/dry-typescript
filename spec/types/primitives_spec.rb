...
RSpec.describe "Primitives" do
  ...
  shared_examples_for "a primitive type" do |type_name, type_def|
    describe "for Types::#{type_name}" do
      let(:types_module) { module_double(type_name => DryTypes.const_get(type_name)) }
      ...
      context "with .optional" do
        let(:types_module) { module_double(type_name => DryTypes.const_get(type_name).optional) }
      ...
      context "in an array" do
        let(:types_module) { module_double("#{type_name}Array" => DryTypes::Array.of(DryTypes.const_get(type_name))) }
  ...
  describe "Types::Nil" do
    let(:types_module) { module_double(Nil: DryTypes::Nil) }
    ...
    context "with .optional" do
      let(:types_module) { module_double(Nil: DryTypes::Nil.optional) }
  ...
  describe "Types::Array" do
    let(:types_module) { module_double(MyArray: DryTypes::Array) }
    ...
    context "with .optional" do
      let(:types_module) { module_double(OptionalArray: DryTypes::Array.optional) }
