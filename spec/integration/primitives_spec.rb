...
RSpec.describe "Primitives" do
  ...
  shared_examples_for "a primitive type" do |dry_types, type_name, type_def|
    describe "for Types::#{type_name}" do
      let(:namespace) { ts_namespace(type_name => dry_types.const_get(type_name)) }
      ...
      context "with .optional" do
        let(:namespace) { ts_namespace(type_name => dry_types.const_get(type_name).optional) }
      ...
      context "in an array" do
        let(:namespace) { ts_namespace("#{type_name}Array" => dry_types::Array.of(dry_types.const_get(type_name))) }
  ...
  describe "Types::Nil" do
    let(:namespace) { ts_namespace(Nil: Dry::Typescript::DryTypes::Nil) }
    ...
    context "with .optional" do
      let(:namespace) { ts_namespace(Nil: Dry::Typescript::DryTypes::Nil.optional) }
  ...
  describe "Types::Array" do
    let(:namespace) { ts_namespace(MyArray: Dry::Typescript::DryTypes::Array) }
    ...
    context "with .optional" do
      let(:namespace) { ts_namespace(OptionalArray: Dry::Typescript::DryTypes::Array.optional) }
